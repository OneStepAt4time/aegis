...
- Multi-agent consensus (parallel review → verdict)

> **Note:** Pipelines are experimental. Stage timeouts and retry policies are not yet configurable (see issue #1423).

---

## Consensus Review

Multi-agent review where 2-3 Claude Code sessions independently evaluate the same work, then produce a consensus verdict.

### Create a Consensus Review

\`\`\`bash
curl -X POST http://localhost:9100/v1/sessions/{id}/consensus \
  -H "Authorization: Bearer $AEGIS_AUTH_TOKEN"
\`\`\`

Claude Code reviews the session's work across three dimensions:
...
\`\`\`bash
curl http://localhost:9100/v1/consensus/{review-id} \
  -H "Authorization: Bearer $AEGIS_AUTH_TOKEN"
\`\`\`
  ...
  "findings": [
    {
      ...
      "location": "src/handlers/user.ts:47"
    }
  ]
}
\`\`\`

**Verdict values:** \`approved\`, \`needs_revision\`, \`rejected\`

### SSE Events

Subscribe to \`consensus.completed\` via SSE to be notified when the review finishes.

> **Note:** Consensus reviews run in parallel sessions. Ensure \`AEGIS_MAX_CONCURRENT_SESSIONS\` is set to at least 4 for consensus workflows.
