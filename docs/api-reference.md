...
> **Note:** The `model` field is populated at creation time — either from an explicit request parameter or auto-detected from Claude Code settings (`ANTHROPIC_MODEL` in `.claude/settings.local.json` or `.claude/settings.json`). The `effort` field appears when provided at creation time. The `isolationMode` field (`"worktree"` or `"none"`) is derived from Claude Code settings or the server `isolationPolicy`. When `"worktree"`, Aegis creates a git worktree for the session at `.claude/worktrees/<session-id-prefix>` on branch `session/<id-prefix>` and the session's `workDir` becomes the worktree path, preventing uncommitted changes from polluting the main workspace. When `"none"`, the session edits the project directory directly. The `runnerName` field identifies the agent type (e.g. `"claude-code"` for ACP sessions); it is absent for legacy sessions.
>
...
Returns aggregated session statistics. Non-admin keys see only their own. The `active` count **excludes terminal sessions** (`killed`, `completed`, `crashed`); use `total` for all sessions or `byStatus` for a breakdown. The `totalFailed` count **excludes killed sessions** — killed sessions are tracked separately in `totalKilled`.
