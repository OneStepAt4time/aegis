...
### `ag update` — Self-Update

Check for and apply Aegis updates. Auto-detects install method (npm global vs direct binary).
...
3. If update available, confirms with user (unless `--yes`)
4. Updates via `npm update -g` (npm installs) or downloads + SHA-256 verified binary (direct installs)
...
Authenticate with an OIDC identity provider using the device authorization flow (RFC 8628).

```bash
ag login                              # Interactive — opens browser
ag login --server http://aegis:9100    # Override server URL
ag login --no-open                    # Print URL instead of opening browser
ag login --json                       # Machine-readable output
```

Requires `AEGIS_OIDC_ISSUER` and `AEGIS_OIDC_CLIENT_ID` to be configured. If OIDC is not set up, the command prints an error with setup instructions.

**Flow:**
1. Discovers OIDC endpoints from the issuer's `.well-known/openid-configuration`
2. Requests a device code from the IdP
3. Prints a verification URL and code
4. Polls the IdP until you authorize in the browser
5. Stores tokens in `~/.aegis/auth/`
...
| `1` | Error (auth failed, timeout, IdP unreachable) |
| `2` | Config error (OIDC not configured) |

### `ag logout` — Remove Stored Credentials

Remove stored OIDC credentials. Attempts token revocation at the IdP (best-effort).
...
Display the currently authenticated identity, role, and token expiry. Automatically refreshes expired tokens if a refresh token is available.
...
| Code | Meaning |
|------|----------|
| `0` | Success — identity displayed |
| `1` | Error (not authenticated, token refresh failed) |
...
Most commands use standard exit codes. Specific overrides are documented per-command.

| Code | General Meaning |
|------|----------------|
| `0` | Success |
| `1` | Error (server failed, session creation failed, auth error) |
| `2` | Special — command-specific (see per-command docs) |

**Command-specific overrides:**

| Command | Code 2 Meaning |
|---------|---------------|
| `ag init` | Aegis already running (prints existing URL) |
| `ag run` | Rate limit hit (HTTP 429 from server) |
| `ag login` | Config error (OIDC not configured) |
| `ag update --check` | Update available (used for scripting) |
...
ag list                List sessions
ag list --all          Include killed/completed/crashed
ag list --json         JSON output
