...
- **Telegram group env var name in docs** — `docs/guides/phone-approvals.md`, `docs/integrations/notifications.md`, and `docs/integrations/cli.md` now reference `AEGIS_TG_GROUP` (matches `src/config/index.ts:443`) instead of the unrecognized `AEGIS_TG_GROUP_ID`; the previous name was silently ignored at startup (closes [#4609](https://github.com/OneStepAt4time/aegis/issues/4609))
- **CLI startup hint `ag telegram`** — `src/cli.ts:82` banner now points users at `AEGIS_TG_BOT_TOKEN` / `AEGIS_TG_GROUP` and the `docs/guides/phone-approvals.md` walkthrough instead of the non-existent `ag telegram` subcommand (closes [#4610](https://github.com/OneStepAt4time/aegis/issues/4610))
- **Telegram bot env var name correction** — `src/cli.ts:82` startup banner, `src/__tests__/cli-json-logs.test.ts:222` assertion, `docs/guides/phone-approvals.md`, `docs/five-minute-setup.md`, `docs/onboarding.md`, `docs/integrations/notifications.md`, and `docs/integrations/cli.md` now reference `AEGIS_TG_TOKEN` (matches `src/config/index.ts:442`) instead of the unrecognized `AEGIS_TG_BOT_TOKEN`; the previous name was silently ignored at startup, breaking the 5-minute phone-approval setup for new users (closes [#4638](https://github.com/OneStepAt4time/aegis/issues/4638))
...
- **Viewer RBAC isolation** — tenant-scoped key enumeration, viewer cannot access admin endpoints ([#3382](https://github.com/OneStepAt4time/aegis/pull/3382))
- **File permissions hardened** — removed info leaks in state directory ([#3375](https://github.com/OneStepAt4time/aegis/pull/3375))
- **API key name validation** — reject unsafe characters in key names ([#3378](https://github.com/OneStepAt4time/aegis/pull/3378))
- **RBAC role guards** — requireRole checks on templates and tools endpoints ([#3207](https://github.com/OneStepAt4time/aegis/pull/3207))
- **strictRBAC config** — enforce RBAC even when auth is disabled ([#3208](https://github.com/OneStepAt4time/aegis/pull/3208))
- **Telegram HTML sanitizer hardened** — escape single quotes, sanitize hrefs and callback_data ([#3219](https://github.com/OneStepAt4time/aegis/pull/3219))
- **XSS in esc()** — escape double quotes to prevent attribute injection ([#3209](https://github.com/OneStepAt4time/aegis/pull/3209))
- **Dashboard static rate limiting** — per-IP fixed-window limiter for asset routes ([#3220](https://github.com/OneStepAt4time/aegis/pull/3220))
- **StaticRateLimiter prune timer** — periodic cleanup to prevent memory growth ([#3227](https://github.com/OneStepAt4time/aegis/pull/3227))
- **Auth on sensitive endpoints** — requireRole on GET /v1/sessions, auth on /metrics, dead-letter, channels/health
- **Session ID enumeration prevention** — unauthorized sessions return 404 instead of 403
- **workDir security** — reject system temp dirs, file paths, and empty prompts
- **PATCH /v1/auth/keys/:id** — update role, name, permissions with self-demotion guard ([#3207](https://github.com/OneStepAt4time/aegis/pull/3207))
- **Auth token discovery mismatch resolved** between CLI and server ([#3409](https://github.com/OneStepAt4time/aegis/pull/3409))
- **Proactive keys.json reload** + orphan auth-token detection ([#3484](https://github.com/OneStepAt4time/aegis/pull/3484))
- **Auth token persistence** — write to ~/.aegis/auth-token ([#3369](https://github.com/OneStepAt4time/aegis/pull/3369), [#3386](https://github.com/OneStepAt4time/aegis/pull/3386))
...
- **Audit directory creation** — ensure exists before file lock acquisition ([#3380](https://github.com/OneStepAt4time/aegis/pull/3380))
- **protobufjs override** to >=8.2.0 — resolves 7 HIGH CVEs ([#3218](https://github.com/OneStepAt4time/aegis/pull/3218))
- **SOC2 CC mapping updated** with RBAC fixes, OIDC, audit v4 ([#3205](https://github.com/OneStepAt4time/aegis/pull/3205))

### Features

- **Cost tracking API** — per-session cost, burn rate, cache-hit rate via GET /v1/cost/* endpoints ([#3264](https://github.com/OneStepAt4time/aegis/pull/3264))
- **Runner abstraction layer** — pluggable agent backend interface ([#3263](https://github.com/OneStepAt4time/aegis/pull/3263))
- **Conversational onboarding** — ag init with --model, --name, identity scaffolding ([#3265](https://github.com/OneStepAt4time/aegis/pull/3265))
...
- **Per-day token breakdown endpoint** — GET /v1/analytics/tokens with dailyBreakdown ([#3282](https://github.com/OneStepAt4time/aegis/pull/3282))
- **Dashboard multi-step onboarding wizard** — visual first-run experience
- **Dashboard Getting Started card** — welcome card for new users ([#3411](https://github.com/OneStepAt4time/aegis/pull/3411))
- **Structured learnings system** — pattern-based learning extraction ([#3413](https://github.com/OneStepAt4time/aegis/pull/3413))
- **--yes flag for ag run** — non-interactive flow for CI/CD ([#3181](https://github.com/OneStepAt4time/aegis/pull/3181))
- **CLI session subcommands** — ag list, ag read, ag kill, ag status, ag tail ([#3499](https://github.com/OneStepAt4time/aegis/pull/3499))
- **Zero-config localhost** — ag init no longer requires a token on localhost ([#3496](https://github.com/OneStepAt4time/aegis/pull/3496))
- **Zero-config Quick Start** — one-command ag run flow without any setup ([#3503](https://github.com/OneStepAt4time/aegis/pull/3503), [#3517](https://github.com/OneStepAt4time/aegis/pull/3517))
- **Dashboard zero-config auth detection** — auto-detect auth on localhost ([#3493](https://github.com/OneStepAt4time/aegis/pull/3493))
- **Dashboard friendlier display names** — session names derived from prompts ([#3491](https://github.com/OneStepAt4time/aegis/pull/3491))
- **Dashboard aria-label i18n** — 152+ translation keys extracted across 3 batches ([#3229](https://github.com/OneStepAt4time/aegis/pull/3229))
- **Dashboard design tokens** — system-wide color migration, centralized Recharts theme ([#3456](https://github.com/OneStepAt4time/aegis/pull/3456), [#3455](https://github.com/OneStepAt4time/aegis/pull/3455))
- **Dashboard wire cost panels** to real /v1/cost/* API endpoints
- **Sessions route cleanup** + ACP backend JSDoc ([#3305](https://github.com/OneStepAt4time/aegis/pull/3305))

### Bug Fixes — ACP

- **sendPrompt uses request()** with 5s ack timeout instead of notify() ([#3479](https://github.com/OneStepAt4time/aegis/pull/3479))
- **Process group kill** — kill entire process group on session shutdown ([#3250](https://github.com/OneStepAt4time/aegis/pull/3250))
- **ACP backend shutdown on kill** — reap CC child processes on single-session kill ([#3224](https://github.com/OneStepAt4time/aegis/pull/3224))
- **Persist() failure recovery** — don't cascade errors on state persistence failure ([#3383](https://github.com/OneStepAt4time/aegis/pull/3383))
- **inFlightPrompts cleared on dispose** — prevent stale prompts after runtime disposal ([#3435](https://github.com/OneStepAt4time/aegis/pull/3435))
- **bypassPermissions activated** in ACP settings ([#3436](https://github.com/OneStepAt4time/aegis/pull/3436))
- **Wire ACP notifications** to event store for /read endpoint ([#3422](https://github.com/OneStepAt4time/aegis/pull/3422), [#3463](https://github.com/OneStepAt4time/aegis/pull/3463))
- **TenantId fallback aligned** with SYSTEM_TENANT for unauth requests ([#3237](https://github.com/OneStepAt4time/aegis/pull/3237))
- **Prompt timeout increased** to 60s in action dispatch ([#3333](https://github.com/OneStepAt4time/aegis/pull/3333))
- **JSON-RPC timeout configurable** via AEGIS_ACP_PROMPT_TIMEOUT_MS ([#3223](https://github.com/OneStepAt4time/aegis/pull/3223))
- **Default timeout 60s→120s** for BYO-LLM proxies ([#3243](https://github.com/OneStepAt4time/aegis/pull/3243))
- **Async prompt delivery reverted** — P0 regression ([#3271](https://github.com/OneStepAt4time/aegis/pull/3271))
- **Monitor routes assistant messages** for ACP .claude workdirs ([#3286](https://github.com/OneStepAt4time/aegis/pull/3286))
- **SessionMonitor started** in container boot ([#3189](https://github.com/OneStepAt4time/aegis/pull/3189))

### Bug Fixes — CLI

- **--accept-permissions flag** for ag run and ag create
- **--port flag propagation** to config and server process ([#3343](https://github.com/OneStepAt4time/aegis/pull/3343))
- **ag run --help** shows run-specific flags ([#3358](https://github.com/OneStepAt4time/aegis/pull/3358))
- **CC auth preflight check** in ag run ([#3381](https://github.com/OneStepAt4time/aegis/pull/3381))
- **Preflight auth prevents orphaned sessions** ([#3306](https://github.com/OneStepAt4time/aegis/pull/3306), [#3314](https://github.com/OneStepAt4time/aegis/pull/3314))
- **Project-local config** — ag run finds .aegis/config.yaml ([#3307](https://github.com/OneStepAt4time/aegis/pull/3307), [#3313](https://github.com/OneStepAt4time/aegis/pull/3313))
- **Auth bootstrap via AuthManager** in ag run ([#3261](https://github.com/OneStepAt4time/aegis/pull/3261))
- **Timeouts on ag create** session/send requests ([#3424](https://github.com/OneStepAt4time/aegis/pull/3424))
- **Timeout on ag run** session creation fetch ([#3247](https://github.com/OneStepAt4time/aegis/pull/3247))
- **EADDRINUSE detection** via PID file fallback ([#3388](https://github.com/OneStepAt4time/aegis/pull/3388))
- **Streaming output fix** — read correct field names ([#3389](https://github.com/OneStepAt4time/aegis/pull/3389))
- **ag init --force** — implies --yes, handles partial state ([#3391](https://github.com/OneStepAt4time/aegis/pull/3391))
- **ag init --force replaces admin key** instead of crashing ([#3384](https://github.com/OneStepAt4time/aegis/pull/3384))

### Bug Fixes — API

- **POST /v1/sessions/:id/prompt** — route alias for /send ([#3420](https://github.com/OneStepAt4time/aegis/pull/3420))
- **/send returns reply.send()** for success response ([#3442](https://github.com/OneStepAt4time/aegis/pull/3442))
- **/send returns immediately** instead of blocking ([#3437](https://github.com/OneStepAt4time/aegis/pull/3437))
- **kill returns body** instead of empty response ([#3425](https://github.com/OneStepAt4time/aegis/pull/3425))
- **GET /v1/sessions/:id/terminal/content** — dashboard LiveTerminal endpoint ([#3429](https://github.com/OneStepAt4time/aegis/pull/3429))
- **Metering lifecycle wired** — call metering.start() for session tracking ([#3335](https://github.com/OneStepAt4time/aegis/pull/3335))
- **Active session count** excludes killed/completed/crashed ([#3334](https://github.com/OneStepAt4time/aegis/pull/3334), [#3396](https://github.com/OneStepAt4time/aegis/pull/3396))
...
- **Light theme contrast overrides** — amber-200, bg-white, border-white ([#3394](https://github.com/OneStepAt4time/aegis/pull/3394), [#3410](https://github.com/OneStepAt4time/aegis/pull/3410))
- **44px touch targets** for interactive elements ([#3393](https://github.com/OneStepAt4time/aegis/pull/3393))
- **API error states differentiated** — offline, timeout, server, unauthorized ([#3373](https://github.com/OneStepAt4time/aegis/pull/3373), [#3395](https://github.com/OneStepAt4time/aegis/pull/3395))
- **SessionDetailPage tab bar scrollable** on mobile ([#3400](https://github.com/OneStepAt4time/aegis/pull/3400), [#3406](https://github.com/OneStepAt4time/aegis/pull/3406))
- **Mobile overflow** — responsive min-widths and grid columns ([#3408](https://github.com/OneStepAt4time/aegis/pull/3408))
- **Resilient API handling** + chart polish ([#3439](https://github.com/OneStepAt4time/aegis/pull/3439))
- **Design tokens migration** — slate-* colors to CSS custom properties ([#3421](https://github.com/OneStepAt4time/aegis/pull/3421))
- **promptDelivery.status added** to schema, types, and equality check ([#3256](https://github.com/OneStepAt4time/aegis/pull/3256))
- **pruneInterval.unref() guard** with typeof check ([#3444](https://github.com/OneStepAt4time/aegis/pull/3444))
- **96 buttons across 36 files** — add type='button' to prevent form submission
- **Aria-label i18n batch 2** — replace hardcoded labels with t() calls
- **Aria-label i18n batch 3** — extract remaining hardcoded labels ([#3229](https://github.com/OneStepAt4time/aegis/pull/3229))
- **Cost API mock polish** for a11y tests
- **Wire cost panels** to real /v1/cost/* API endpoints

### Bug Fixes — Other

- **install-systemd.sh** — prevent template drift in systemd units ([#3486](https://github.com/OneStepAt4time/aegis/pull/3486))
- **ExecStartPre build step** in aegis.service ([#3480](https://github.com/OneStepAt4time/aegis/pull/3480))
- **CI discord-notify** — replace broken TRUSTED grep with case statement ([#3330](https://github.com/OneStepAt4time/aegis/pull/3330))
- **Flaky test isolation** eliminated ([#3303](https://github.com/OneStepAt4time/aegis/pull/3303), [#3304](https://github.com/OneStepAt4time/aegis/pull/3304))
- **Broken tests skipped** to unblock CI ([#3449](https://github.com/OneStepAt4time/aegis/pull/3449), [#3454](https://github.com/OneStepAt4time/aegis/pull/3454))
- **Sessions route cleanup** refactor + ACP JSDoc ([#3302](https://github.com/OneStepAt4time/aegis/pull/3302), [#3305](https://github.com/OneStepAt4time/aegis/pull/3305))
- **Headless ag mcp smoke script** for npx/local usage ([#3328](https://github.com/OneStepAt4time/aegis/pull/3328))
- **feat-minor-bump-gate** uses live PR title ([#3240](https://github.com/OneStepAt4time/aegis/pull/3240))
- **SDK Auto-Sync** PR-based flow instead of direct push ([#3215](https://github.com/OneStepAt4time/aegis/pull/3215))

### Documentation

- **Zero-config Quick Start README** ([#3503](https://github.com/OneStepAt4time/aegis/pull/3503), [#3517](https://github.com/OneStepAt4time/aegis/pull/3517))
- **ADR-0029** — Solo Developer First, Phase 4 Deferred ([#3488](https://github.com/OneStepAt4time/aegis/pull/3488))
- **Install-systemd.sh deployment guide** ([#3487](https://github.com/OneStepAt4time/aegis/pull/3487))
- **Agent dispatch rules** and handoff policy in AGENTS.md ([#3414](https://github.com/OneStepAt4time/aegis/pull/3414))
- **CLI session subcommands docs** — ag list, read, kill, status, tail ([#3508](https://github.com/OneStepAt4time/aegis/pull/3508), [#3510](https://github.com/OneStepAt4time/aegis/pull/3510))
- **Zero-config localhost note** for ag init ([#3507](https://github.com/OneStepAt4time/aegis/pull/3507))
...
- **Competitive matrix updates** — Roo Code shutdown, Cline Enterprise, ClaudeClaw ([#3466](https://github.com/OneStepAt4time/aegis/pull/3466), [#3467](https://github.com/OneStepAt4time/aegis/pull/3467))
- **RBAC role tables** for 14 endpoints ([#3319](https://github.com/OneStepAt4time/aegis/pull/3319))
- **Known-issues.md** with workarounds for 5 live bugs ([#3438](https://github.com/OneStepAt4time/aegis/pull/3438))
- **Hooks UX** — curl examples, 401 error table, hook secret lookup ([#3450](https://github.com/OneStepAt4time/aegis/pull/3450))
- **Lifecycle hooks guide** expanded with competitive positioning ([#3317](https://github.com/OneStepAt4time/aegis/pull/3317))
- **Freshness audit** — getting-started, architecture, competitive matrix ([#3322](https://github.com/OneStepAt4time/aegis/pull/3322))
- **Per-day token breakdown** docs with from/to params ([#3288](https://github.com/OneStepAt4time/aegis/pull/3288))
- **Cost tracking + onboarding + runner docs** ([#3272](https://github.com/OneStepAt4time/aegis/pull/3272))
- **Async prompt delivery revert docs** ([#3274](https://github.com/OneStepAt4time/aegis/pull/3274), [#3277](https://github.com/OneStepAt4time/aegis/pull/3277))
- **Auth prerequisite** for ag create and ag run ([#3370](https://github.com/OneStepAt4time/aegis/pull/3370))
- **Claude auth + ag run flags** docs ([#3371](https://github.com/OneStepAt4time/aegis/pull/3371))
- **RBAC viewer + key name docs** ([#3385](https://github.com/OneStepAt4time/aegis/pull/3385))
- **ag init restart warning** ([#3387](https://github.com/OneStepAt4time/aegis/pull/3387))
- **Known issues for ag create** — silent output + arg shorthand ([#3390](https://github.com/OneStepAt4time/aegis/pull/3390))
- **/send vs /prompt naming** clarification ([#3344](https://github.com/OneStepAt4time/aegis/pull/3344), [#3392](https://github.com/OneStepAt4time/aegis/pull/3392))
- **Stats active count + --accept-permissions** flag ([#3417](https://github.com/OneStepAt4time/aegis/pull/3417))
- **Terminal content + kill format** endpoint docs ([#3458](https://github.com/OneStepAt4time/aegis/pull/3458))
- **Stale /approve /reject paths** removed ([#3464](https://github.com/OneStepAt4time/aegis/pull/3464))
- **Approve/reject 500 docs** ([#3470](https://github.com/OneStepAt4time/aegis/pull/3470))
- **Terminal states docs** — killed, completed, crashed
- **Pending status docs** in API reference and README
- **SOC2 compliance update** ([#1949](https://github.com/OneStepAt4time/aegis/pull/1949), [#3205](https://github.com/OneStepAt4time/aegis/pull/3205))
- **PATCH /v1/auth/keys/:id + strictRBAC** docs ([#3213](https://github.com/OneStepAt4time/aegis/pull/3213))
- **ACP prompt timeout docs** — AEGIS_ACP_PROMPT_TIMEOUT_MS
- **ACP timeout default 60s→120s** docs ([#3245](https://github.com/OneStepAt4time/aegis/pull/3245))
- **Async prompt delivery docs** ([#3253](https://github.com/OneStepAt4time/aegis/pull/3253), [#3254](https://github.com/OneStepAt4time/aegis/pull/3254))
- **Quickstart restructured** for 2-command install flow
- **Competitive threat matrix** — May 12 market pulse + ClaudeClaw
- **/prompt alias for /send** docs
- **--yes flag** in ag run CLI reference
- **ag run troubleshooting** note
- **Changelog entries** for multiple PR batches ([#3228](https://github.com/OneStepAt4time/aegis/pull/3228))
- **Sync OpenAPI spec** for strictRBAC + PATCH auth keys
- **Auto-sync generated TS SDK** ([#3214](https://github.com/OneStepAt4time/aegis/pull/3214))
- **Dashboard static rate limiting** in enterprise docs
- **ACP tenantId alignment** docs

### Dependencies & Chore

- **Version bump** 0.6.6 → 0.6.7 ([#3495](https://github.com/OneStepAt4time/aegis/issues/3495))
- **Version bump** to 0.6.7-preview.1 ([#3203](https://github.com/OneStepAt4time/aegis/pull/3203))
- **MCP SDK, fastify, zod, nodemailer** bumped
