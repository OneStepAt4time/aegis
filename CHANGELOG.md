...
- **`ag run` command** — zero-to-session in one command; creates config, authenticates, and starts an interactive session ([#3050](https://github.com/OneStepAt4time/aegis/pull/3050), closes [#3043](https://github.com/OneStepAt4time/aegis/issues/3043))
- **`ag init --force` flag** — overwrite existing config file without prompting; config-exists warning on normal init ([#3035](https://github.com/OneStepAt4time/aegis/pull/3035), closes [#3028](https://github.com/OneStepAt4time/aegis/issues/3028))
- **Session export endpoint** — `GET /v1/sessions/:id/export` downloads transcript as JSONL or Markdown with Content-Disposition header ([#3153](https://github.com/OneStepAt4time/aegis/pull/3153), closes [#3114](https://github.com/OneStepAt4time/aegis/issues/3114))
- **Dashboard session health alert banner** — real-time health indicator in session overview ([#3033](https://github.com/OneStepAt4time/aegis/pull/3033), closes [#3000](https://github.com/OneStepAt4time/aegis/issues/3000))
- **Dashboard ContextWindowMeter component** — visual context window usage meter in session detail ([#3141](https://github.com/OneStepAt4time/aegis/pull/3141), closes [#3127](https://github.com/OneStepAt4time/aegis/issues/3127))
...
- **Cost tracking API** — burn rate, cache-hit rate, per-session cost with GET /v1/cost/* endpoints ([#3264](https://github.com/OneStepAt4time/aegis/pull/3264))
- **Runner abstraction layer** — pluggable agent backend interface ([#3263](https://github.com/OneStepAt4time/aegis/pull/3263))
- **ag init conversational onboarding** — --model, --name, identity scaffolding ([#3265](https://github.com/OneStepAt4time/aegis/pull/3265))
- **Dashboard cost analytics panels** — burn rate, token breakdown, cost by model ([#3275](https://github.com/OneStepAt4time/aegis/pull/3275))
- **Dashboard agent contributions panel** — per-agent git attribution ([#3276](https://github.com/OneStepAt4time/aegis/pull/3276))
- **Per-day token breakdown endpoint** — dailyBreakdown with from/to params ([#3282](https://github.com/OneStepAt4time/aegis/pull/3282))
- **Structured learnings system** — capture and retrieve operational learnings ([#3413](https://github.com/OneStepAt4time/aegis/pull/3413))
- **Dashboard Getting Started card** — welcome card for new users ([#3411](https://github.com/OneStepAt4time/aegis/pull/3411))
...
- **ACP enabled by default** — `acpEnabled` defaults to `true`; sessions no longer stuck at pending on fresh installs ([#3083](https://github.com/OneStepAt4time/aegis/pull/3083), [#3109](https://github.com/OneStepAt4time/aegis/pull/3109), closes [#3068](https://github.com/OneStepAt4time/aegis/issues/3068), [#3078](https://github.com/OneStepAt4time/aegis/issues/3078))
- **CLI config key** — `ag init` now writes `authToken` instead of deprecated `clientAuthToken` ([#3088](https://github.com/OneStepAt4time/aegis/pull/3088), closes [#3085](https://github.com/OneStepAt4time/aegis/issues/3085))
- **Session name/label validation** — special characters rejected in session `name` and `label` fields; `SAFE_NAME_RE` pattern enforced ([#3095](https://github.com/OneStepAt4time/aegis/pull/3095), closes [#3091](https://github.com/OneStepAt4time/aegis/issues/3091))
- **Batch-delete status enum** — `pending` and `rate_limit` added to valid status values for batch-delete filtering ([#3100](https://github.com/OneStepAt4time/aegis/pull/3100), closes [#3075](https://github.com/OneStepAt4time/aegis/issues/3075))
- **RBAC kill permission** — kill permission checked before session ownership evaluation ([#3101](https://github.com/OneStepAt4time/aegis/pull/3101), closes [#3082](https://github.com/OneStepAt4time/aegis/issues/3082))
- **ACP status preservation** — session status preserved in ACP mode instead of overwritten with `idle` ([#3102](https://github.com/OneStepAt4time/aegis/pull/3102), closes [#3081](https://github.com/OneStepAt4time/aegis/issues/3081))
- **Dashboard static plugin extraction** — dashboard serving extracted from monolithic server into dedicated plugin ([#3159](https://github.com/OneStepAt4time/aegis/pull/3159), closes [#3154](https://github.com/OneStepAt4time/aegis/issues/3154))
- **Dashboard a11y** — replaced hardcoded light-mode colors with CSS variables across 10 pages for dark/light theme compatibility ([#2966](https://github.com/OneStepAt4time/aegis/pull/2966))
- **Dead consensus.ts removed** — deleted unused `consensus.ts` module ([#3117](https://github.com/OneStepAt4time/aegis/pull/3117), closes [#3112](https://github.com/OneStepAt4time/aegis/issues/3112))
- **Auth token single source of truth** — consolidated client token handling ([#3516](https://github.com/OneStepAt4time/aegis/pull/3516))
- **Sessions route cleanup** — backend route refactor + ACP backend JSDoc ([#3305](https://github.com/OneStepAt4time/aegis/pull/3305))
- **Dashboard design tokens** — system-wide color token adoption ([#3456](https://github.com/OneStepAt4time/aegis/pull/3456))
- **Centralized Recharts theme** — chart polish with design tokens ([#3455](https://github.com/OneStepAt4time/aegis/pull/3455))
...
- **Auth on sensitive endpoints** — `GET /v1/sessions` now requires role check ([#2992](https://github.com/OneStepAt4time/aegis/pull/2992), closes [#2975](https://github.com/OneStepAt4time/aegis/issues/2975)); `/metrics`, dead-letter, and `channels/health` endpoints now require auth ([#2994](https://github.com/OneStepAt4time/aegis/pull/2994), closes [#2976](https://github.com/OneStepAt4time/aegis/issues/2976), [#2977](https://github.com/OneStepAt4time/aegis/issues/2977))
- **Session ID enumeration prevention** — unauthorized sessions return 404 instead of 403 to prevent ID scanning ([#3146](https://github.com/OneStepAt4time/aegis/pull/3146), closes [#3071](https://github.com/OneStepAt4time/aegis/issues/3071))
- **Duplicate API key names rejected** — prevents duplicate key names within the same tenant ([#3106](https://github.com/OneStepAt4time/aegis/pull/3106), closes [#3073](https://github.com/OneStepAt4time/aegis/issues/3073))
- **Dead `audit` permission removed** from RBAC model ([#3017](https://github.com/OneStepAt4time/aegis/pull/3017), closes [#2979](https://github.com/OneStepAt4time/aegis/issues/2979))
- **Auth-fail rate limit reset** — rate limit counter resets on successful authentication ([#3108](https://github.com/OneStepAt4time/aegis/pull/3108), closes [#3072](https://github.com/OneStepAt4time/aegis/issues/3072))
- **Telegram md2html sanitization** — sanitize hrefs, topic names, and `callback_data` to prevent injection ([#3174](https://github.com/OneStepAt4time/aegis/pull/3174), closes [#3173](https://github.com/OneStepAt4time/aegis/issues/3173))
- **Prompt delivery status** — honest status when ACP disabled instead of claiming delivered ([#3022](https://github.com/OneStepAt4time/aegis/pull/3022), closes [#2995](https://github.com/OneStepAt4time/aegis/issues/2995))
- **File workDir and empty prompt rejected** — validation rejects file-path workDirs and empty prompt strings ([#3008](https://github.com/OneStepAt4time/aegis/pull/3008), closes [#2989](https://github.com/OneStepAt4time/aegis/issues/2989), [#2990](https://github.com/OneStepAt4time/aegis/issues/2990))
- **Audit chain validation** — resolves chain validation across rotated log files ([#3019](https://github.com/OneStepAt4time/aegis/pull/3019), closes [#2986](https://github.com/OneStepAt4time/aegis/issues/2986))
- **Worktree data loss prevention** — force `baseRef=head` to prevent silent data loss in git operations ([#3158](https://github.com/OneStepAt4time/aegis/pull/3158), closes [#3155](https://github.com/OneStepAt4time/aegis/issues/3155))
- **Atomic JSON state writes** — all JSON state files written atomically to prevent corruption on crash ([#3047](https://github.com/OneStepAt4time/aegis/pull/3047), closes [#3045](https://github.com/OneStepAt4time/aegis/issues/3045))
- **ACP prompt routing** — prompts routed through ACP runtime instead of stub ([#3096](https://github.com/OneStepAt4time/aegis/pull/3096), closes [#3093](https://github.com/OneStepAt4time/aegis/issues/3093))
- **ACP local storage resilience** — handle empty/malformed local storage file gracefully ([#3037](https://github.com/OneStepAt4time/aegis/pull/3037), closes [#3036](https://github.com/OneStepAt4time/aegis/issues/3036))
- **ACP /read from event store** — `/read` endpoint returns messages from ACP event store ([#3151](https://github.com/OneStepAt4time/aegis/pull/3151), closes [#3143](https://github.com/OneStepAt4time/aegis/issues/3143))
- **ACP child process env** — pass `HOME` and Anthropic env vars to child process; log stderr ([#3147](https://github.com/OneStepAt4time/aegis/pull/3147), [#3148](https://github.com/OneStepAt4time/aegis/pull/3148), closes [#3135](https://github.com/OneStepAt4time/aegis/issues/3135))
- **ACP mcpServers format** — send `mcpServers` as empty array instead of empty object ([#3089](https://github.com/OneStepAt4time/aegis/pull/3089))
- **CLI curl auth header** — include auth header in `ag create` curl output ([#3021](https://github.com/OneStepAt4time/aegis/pull/3021), closes [#2996](https://github.com/OneStepAt4time/aegis/issues/2996))
- **CLI unknown commands** — reject unknown CLI commands instead of silently creating sessions ([#3104](https://github.com/OneStepAt4time/aegis/pull/3104), closes [#3079](https://github.com/OneStepAt4time/aegis/issues/3079))
- **CLI health check retry** — retry health check after spawn failure ([#3150](https://github.com/OneStepAt4time/aegis/pull/3150), closes [#3067](https://github.com/OneStepAt4time/aegis/issues/3067))
- **CLI warning tone** — replaced scary 0-attempts warning with neutral queued message ([#3063](https://github.com/OneStepAt4time/aegis/pull/3063), closes [#3059](https://github.com/OneStepAt4time/aegis/issues/3059))
- **workDir error message** — include allowed directories in validation error message ([#3062](https://github.com/OneStepAt4time/aegis/pull/3062), closes [#3060](https://github.com/OneStepAt4time/aegis/issues/3060))
- **Session create response** — add `name` alias to session create response for discoverability ([#3144](https://github.com/OneStepAt4time/aegis/pull/3144), closes [#3134](https://github.com/OneStepAt4time/aegis/issues/3134))
- **Dashboard design token violations** — resolved 46 design-token gate violations ([#3007](https://github.com/OneStepAt4time/aegis/pull/3007), closes [#2980](https://github.com/OneStepAt4time/aegis/issues/2980))
- **Dashboard CostPage** — fix budget link and CSV export all records ([#3025](https://github.com/OneStepAt4time/aegis/pull/3025), closes [#3015](https://github.com/OneStepAt4time/aegis/issues/3015), [#3012](https://github.com/OneStepAt4time/aegis/issues/3012))
- **Dashboard OverviewPage** — fix non-null assertion crash ([#3027](https://github.com/OneStepAt4time/aegis/pull/3027))
- **Dashboard a11y** — add missing aria-labels to TemplatesPage, AuthKeysPage, SessionHistoryPage ([#2968](https://github.com/OneStepAt4time/aegis/pull/2968)); add aria-label to batch remove button in CreateSessionModal ([#3129](https://github.com/OneStepAt4time/aegis/pull/3129))
- **Dashboard responsive + error boundaries** — responsive breakpoints and error boundaries for 4 pages ([#3130](https://github.com/OneStepAt4time/aegis/pull/3130))
- **Dashboard PWA** — serve `manifest.json` at root without auth ([#3111](https://github.com/OneStepAt4time/aegis/pull/3111), closes [#3092](https://github.com/OneStepAt4time/aegis/issues/3092))
- **Dashboard root redirect** — redirect `/` to `/dashboard/` ([#3110](https://github.com/OneStepAt4time/aegis/pull/3110), closes [#3076](https://github.com/OneStepAt4time/aegis/issues/3076))
- **Server root auth bypass** — skip auth for root `/` redirect ([#3133](https://github.com/OneStepAt4time/aegis/pull/3133))
- **Telegram topic map persistence** — persist topic map and report channel health ([#3171](https://github.com/OneStepAt4time/aegis/pull/3171))
- **Telegram poll timeout** — reduce `getUpdates` long-poll timeout to avoid abort race ([#3142](https://github.com/OneStepAt4time/aegis/pull/3142))
...
- **ACP process group kill** — kill entire process group on session shutdown ([#3250](https://github.com/OneStepAt4time/aegis/pull/3250))
- **ACP prompt delivery** — sendPrompt uses request() with 5s ack timeout ([#3479](https://github.com/OneStepAt4time/aegis/pull/3479))
- **ACP persist recovery** — recover from persist() failure instead of cascading ([#3383](https://github.com/OneStepAt4time/aegis/pull/3383))
- **ACP inFlightPrompts cleanup** — clear when runtime is disposed ([#3435](https://github.com/OneStepAt4time/aegis/pull/3435))
- **ACP async prompt delivery reverted** — P0 regression ([#3271](https://github.com/OneStepAt4time/aegis/pull/3271))
- **ACP session/prompt timeout** — increased to 60s in action dispatch ([#3333](https://github.com/OneStepAt4time/aegis/pull/3333))
- **bypassPermissions in ACP settings** — activate correctly ([#3436](https://github.com/OneStepAt4time/aegis/pull/3436))
- **Auth token discovery mismatch** — CLI vs server token location conflict ([#3409](https://github.com/OneStepAt4time/aegis/pull/3409))
- **Auth token persistence** — persist to ~/.aegis/auth-token file ([#3386](https://github.com/OneStepAt4time/aegis/pull/3386))
- **Auth key reload** — automatic reload for state directory recovery ([#3374](https://github.com/OneStepAt4time/aegis/pull/3374))
- **API key name validation** — reject unsafe characters ([#3378](https://github.com/OneStepAt4time/aegis/pull/3378))
- **RBAC viewer isolation** — tenant-scoped key enumeration ([#3382](https://github.com/OneStepAt4time/aegis/pull/3382))
- **File permissions hardening** — remove info leaks ([#3375](https://github.com/OneStepAt4time/aegis/pull/3375))
- **Audit directory creation** — ensure exists before file lock ([#3380](https://github.com/OneStepAt4time/aegis/pull/3380))
- **CLI ag run auth bootstrap** — use AuthManager ([#3261](https://github.com/OneStepAt4time/aegis/pull/3261))
- **CLI project-local config** — find .aegis/config.yaml ([#3313](https://github.com/OneStepAt4time/aegis/pull/3313))
- **CLI preflight auth** — prevent orphaned sessions ([#3314](https://github.com/OneStepAt4time/aegis/pull/3314))
- **CLI CC auth preflight** — Claude Code auth check in ag run ([#3381](https://github.com/OneStepAt4time/aegis/pull/3381))
- **CLI ag init --force** — replaces key, implies --yes ([#3384](https://github.com/OneStepAt4time/aegis/pull/3384), [#3391](https://github.com/OneStepAt4time/aegis/pull/3391))
- **CLI EADDRINUSE** — PID file fallback for port conflicts ([#3388](https://github.com/OneStepAt4time/aegis/pull/3388))
- **CLI streaming output** — correct field name ([#3389](https://github.com/OneStepAt4time/aegis/pull/3389))
- **CLI --port propagation** — propagates to config and server ([#3343](https://github.com/OneStepAt4time/aegis/pull/3343))
- **CLI ag run --help** — run-specific flags ([#3358](https://github.com/OneStepAt4time/aegis/pull/3358))
- **CLI session timeouts** — ag create session/send request timeouts ([#3424](https://github.com/OneStepAt4time/aegis/pull/3424))
- **API /prompt route alias** — POST /v1/sessions/:id/prompt ([#3420](https://github.com/OneStepAt4time/aegis/pull/3420))
- **API /send response** — return reply.send() for success ([#3442](https://github.com/OneStepAt4time/aegis/pull/3442))
- **API /kill response body** — returns body on POST kill ([#3425](https://github.com/OneStepAt4time/aegis/pull/3425))
- **API approve/reject** — correct paths, reject when no permission pending ([#3428](https://github.com/OneStepAt4time/aegis/pull/3428))
- **API session completion from hooks** — idempotent metrics ([#3427](https://github.com/OneStepAt4time/aegis/pull/3427))
- **API ACP notifications** — wire to event store for /read ([#3463](https://github.com/OneStepAt4time/aegis/pull/3463))
- **API metering lifecycle** — call metering.start() ([#3335](https://github.com/OneStepAt4time/aegis/pull/3335))
- **API active session count** — exclude terminal states ([#3396](https://github.com/OneStepAt4time/aegis/pull/3396))
- **API terminal content** — GET /v1/sessions/:id/terminal/content ([#3429](https://github.com/OneStepAt4time/aegis/pull/3429))
- **Cost tracking bugs** — fix 3 bugs + wire MeteringService lifecycle ([#3311](https://github.com/OneStepAt4time/aegis/pull/3311), [#3315](https://github.com/OneStepAt4time/aegis/pull/3315))
- **Monitor ACP routing** — route assistant messages for .claude workdirs ([#3286](https://github.com/OneStepAt4time/aegis/pull/3286))
- **Dashboard promptDelivery schema** — add status field ([#3256](https://github.com/OneStepAt4time/aegis/pull/3256))
- **Dashboard design token migration** — slate-* colors to tokens ([#3421](https://github.com/OneStepAt4time/aegis/pull/3421))
- **Dashboard light theme** — contrast overrides + full coverage ([#3394](https://github.com/OneStepAt4time/aegis/pull/3394), [#3410](https://github.com/OneStepAt4time/aegis/pull/3410))
- **Dashboard touch targets** — 44px for interactive elements ([#3393](https://github.com/OneStepAt4time/aegis/pull/3393))
- **Dashboard mobile overflow** — responsive min-widths and grid ([#3408](https://github.com/OneStepAt4time/aegis/pull/3408))
- **Dashboard SessionDetailPage** — scrollable tab bar on mobile ([#3406](https://github.com/OneStepAt4time/aegis/pull/3406))
- **Dashboard API error states** — offline, timeout, server, unauthorized ([#3395](https://github.com/OneStepAt4time/aegis/pull/3395))
- **Dashboard resilient API handling** — chart polish + error recovery ([#3439](https://github.com/OneStepAt4time/aegis/pull/3439))
- **Dashboard pruneInterval guard** — typeof check before .unref() ([#3444](https://github.com/OneStepAt4time/aegis/pull/3444))
- **Deploy ExecStartPre** — add build step to aegis.service ([#3480](https://github.com/OneStepAt4time/aegis/pull/3480))
- **Deploy install-systemd.sh** — prevent template drift ([#3486](https://github.com/OneStepAt4time/aegis/pull/3486))
- **CI discord-notify** — replace broken TRUSTED grep with case statement ([#3330](https://github.com/OneStepAt4time/aegis/pull/3330))
...
- **QA sprint link fixes** — fixed 14 broken links, stale MCP count, and phantom resources ([#2993](https://github.com/OneStepAt4time/aegis/pull/2993))
- **API accuracy drift fixes** — corrected security endpoints, validation changes, and prompt delivery status ([#3023](https://github.com/OneStepAt4time/aegis/pull/3023))
- **SSE auth + unknown status docs** — documented SSE token authentication and unknown session status ([#3040](https://github.com/OneStepAt4time/aegis/pull/3040))
- **--force flag docs** — added `--force` flag to CLI reference and SSE auth note to API table ([#3042](https://github.com/OneStepAt4time/aegis/pull/3042))
- **ag run docs** — documented `ag run` command in README, getting-started, and CLI reference ([#3055](https://github.com/OneStepAt4time/aegis/pull/3055))
- **Pending status docs** — documented `pending` session status in API reference and README ([#3065](https://github.com/OneStepAt4time/aegis/pull/3065), [#3066](https://github.com/OneStepAt4time/aegis/pull/3066))
- **Terminal states docs** — documented `killed`, `completed`, `crashed` terminal states ([#3145](https://github.com/OneStepAt4time/aegis/pull/3145))
- **Session export docs** — documented `GET /v1/sessions/:id/export` endpoint ([#3160](https://github.com/OneStepAt4time/aegis/pull/3160))
- **Session export user guide** — added user guide for session export and competitive threat matrix ([#3161](https://github.com/OneStepAt4time/aegis/pull/3161))
- **Config + name/AACP docs fixes** — corrected config example, name/label restrictions, ACP warning field ([#3098](https://github.com/OneStepAt4time/aegis/pull/3098))
- **WorkDir defaults docs** — fixed allowed workDir defaults documentation ([#3084](https://github.com/OneStepAt4time/aegis/pull/3084))
- **Session reuse clarification** — clarified session reuse is workDir-based, not name-based ([#3024](https://github.com/OneStepAt4time/aegis/pull/3024))
...
- **Cost tracking + onboarding + runner docs** — cost API, ag init onboarding, runner abstraction ([#3272](https://github.com/OneStepAt4time/aegis/pull/3272))
- **Async prompt delivery revert docs** — promptDelivery updated to synchronous ([#3277](https://github.com/OneStepAt4time/aegis/pull/3277))
- **RBAC role tables** — RBAC tables for 14 endpoints ([#3319](https://github.com/OneStepAt4time/aegis/pull/3319))
- **Competitive differentiators** — Aegis vs cc-connect vs Verdent AI ([#3320](https://github.com/OneStepAt4time/aegis/pull/3320))
- **"Why Aegis" blog post** — developer blog post (DevRel) ([#3324](https://github.com/OneStepAt4time/aegis/pull/3324))
- **Freshness audit** — getting-started, architecture, competitive matrix ([#3322](https://github.com/OneStepAt4time/aegis/pull/3322))
- **Lifecycle hooks guide expansion** — competitive positioning ([#3317](https://github.com/OneStepAt4time/aegis/pull/3317))
- **Auth prerequisite docs** — ag create and ag run requirements ([#3370](https://github.com/OneStepAt4time/aegis/pull/3370))
- **Claude auth + ag run flags** — prerequisite and flags docs ([#3371](https://github.com/OneStepAt4time/aegis/pull/3371))
- **RBAC viewer + key name docs** — viewer restrictions + validation ([#3385](https://github.com/OneStepAt4time/aegis/pull/3385))
- **ag init restart warning** — server restart for auth keys ([#3387](https://github.com/OneStepAt4time/aegis/pull/3387))
- **Known issues** — ag create silent output + arg shorthand ([#3390](https://github.com/OneStepAt4time/aegis/pull/3390))
- **/send vs /prompt naming** — endpoint naming clarification ([#3392](https://github.com/OneStepAt4time/aegis/pull/3392))
- **Stats active count + --accept-permissions** — count clarification + flag ([#3417](https://github.com/OneStepAt4time/aegis/pull/3417))
- **Agent dispatch rules** — dispatch rules and handoff policy ([#3414](https://github.com/OneStepAt4time/aegis/pull/3414))
- **Known-issues.md** — workarounds for 5 live bugs ([#3438](https://github.com/OneStepAt4time/aegis/pull/3438))
- **Hooks UX docs** — curl examples, 401 error table, hook secret lookup ([#3450](https://github.com/OneStepAt4time/aegis/pull/3450))
- **Terminal content + kill format** — new endpoint docs ([#3458](https://github.com/OneStepAt4time/aegis/pull/3458))
- **Stale /approve /reject paths** — remove stale references ([#3464](https://github.com/OneStepAt4time/aegis/pull/3464))
- **Competitive matrix updates** — Roo Code shutdown + Cline Enterprise ([#3466](https://github.com/OneStepAt4time/aegis/pull/3466), [#3467](https://github.com/OneStepAt4time/aegis/pull/3467))
- **Approve/reject 500 docs** — no permission pending error ([#3470](https://github.com/OneStepAt4time/aegis/pull/3470))
- **ADR-0029** — Solo Developer First, Phase 4 Deferred ([#3488](https://github.com/OneStepAt4time/aegis/pull/3488))
- **install-systemd.sh** — deployment guide addition ([#3487](https://github.com/OneStepAt4time/aegis/pull/3487))
...
- **Auto-label action pinned** — pin auto-label action to CJS-compatible deps; regenerate lockfile ([#3002](https://github.com/OneStepAt4time/aegis/pull/3002), [#3044](https://github.com/OneStepAt4time/aegis/pull/3044), closes [#2978](https://github.com/OneStepAt4time/aegis/issues/2978))
- **Trivy + node + lint** — pin trivy-action, upgrade setup-node, optimize lint install ([#3122](https://github.com/OneStepAt4time/aegis/pull/3122))
- **Helm chart dedup** — deduplicate Helm chart build in release pipeline ([#3124](https://github.com/OneStepAt4time/aegis/pull/3124))
- **Release dry-run** — restrict release dry-run trigger, remove duplicate dashboard-test step ([#3123](https://github.com/OneStepAt4time/aegis/pull/3123))
- **Dependabot bumps** — `dorny/paths-filter` v4, `@types/node` 25.6.2, `vite` 8.0.11, `@opentelemetry/exporter-trace-otlp-grpc` 0.217.0, `tailwindcss` 4.3.0 ([#3162](https://github.com/OneStepAt4time/aegis/pull/3162)–[#3167](https://github.com/OneStepAt4time/aegis/pull/3167))
