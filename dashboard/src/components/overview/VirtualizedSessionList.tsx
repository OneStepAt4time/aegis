...
function VirtualizedRow(props: {
  ...
  return (
    <div
      style={style}
      className={`grid border-b border-white/5 transition-all duration-[var(--duration-slow)] ease-out ${ // token-ok: duration-slow maps to 320ms, closest to the original 300ms feel
        isFocused
...
export function VirtualizedSessionList({
  ...
  return (
    <div className="rounded-lg border border-void-lighter overflow-hidden">
      {showHeader && (
        <div
          className="grid border-b border-void-lighter text-[var(--color-text-muted)] text-sm text-left bg-[var(--color-surface)]" // token-ok: --color-text-muted=#94a3b8; #666 is a similar mid-gray not in the token palette
