...
describe('Session Lifecycle Integration Tests', () => {
  ...
  beforeEach(async () => {
    ...
    app.post('/v1/sessions', async (request: any) => {
      const { displayName, workDir, prompt } = request.body || {};
  ...
  it('POST /v1/sessions creates a session', async () => {
    const res = await app.inject({
      method: 'POST',
      url: '/v1/sessions',
      payload: { displayName: 'test', workDir: '/tmp', prompt: 'Hello' },
    });
  ...
  it('POST /v1/sessions rejects missing workDir', async () => {
    const res = await app.inject({
      method: 'POST',
      url: '/v1/sessions',
      payload: { displayName: 'test', prompt: 'Hello' },
    });
  ...
  it('POST /v1/sessions rejects empty prompt', async () => {
    const res = await app.inject({
      method: 'POST',
      url: '/v1/sessions',
      payload: { displayName: 'test', workDir: '/tmp', prompt: '' },
    });
    expect(res.statusCode).toBe(400);
  });
  ...
  it('GET /v1/sessions filters by status', async () => {
    ...
    sessions.set('s1', { id: 's1', status: 'idle', displayName: 'cc-1', workDir: '/tmp' });
    sessions.set('s2', { id: 's2', status: 'working', displayName: 'cc-2', workDir: '/tmp' });
  ...
  it('full lifecycle: create -> poll -> kill', async () => {
    ...
    const create = await app.inject({
      method: 'POST',
      url: '/v1/sessions',
      payload: { displayName: 'lifecycle-test', workDir: '/tmp', prompt: 'Hi' },
    });
