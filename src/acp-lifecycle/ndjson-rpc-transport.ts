/**
 * NDJSON-based JSON-RPC transport for ACP child process communication.
 * Extracted from acp-lifecycle-probe.ts for maintainability (#4235 step 2).
 *
 * Security audit (2026-05-26): no changes to runtime behavior — pure extraction.
 */

import { type ChildProcessWithoutNullStreams } from 'node:child_process';

import { type AcpCapturedFrame } from '../acp-event-stream.js';
import {
  // Constants
  EXIT_TIMEOUT_MS,
  STDERR_LIMIT_BYTES,
  ...
  type ApprovalHandlingOptions,
} from './acp-lifecycle-types.js';
...
export function requireInitializeResponse(
  message: JsonRpcSuccess<unknown>
): JsonRpcSuccess<import('./acp-lifecycle-types.js').AcpInitializeResult> {
...
export function requireNewSessionResponse(
  message: JsonRpcSuccess<unknown>
): JsonRpcSuccess<import('./acp-lifecycle-types.js').AcpNewSessionResult> {
...
export function requirePromptResponse(message: JsonRpcSuccess<unknown>): JsonRpcSuccess<import('./acp-lifecycle-types.js').AcpPromptResult> {
...
export function requireObjectResponse(message: JsonRpcSuccess<unknown>): JsonRpcSuccess<JsonObject> {
  requireObject(message.result, 'result');
  return { jsonrpc: '2.0', id: message.id, result: message.result as JsonObject };
}
...
function isPermissionOptionKind(value: string): value is AcpPermissionOptionKind {
  return ['allow_once', 'allow_always', 'reject_once', 'reject_always'].includes(value);
}
