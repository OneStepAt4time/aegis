...
export function registerSessionActionRoutes(app: FastifyInstance, ctx: RouteContext): void {
  ...
  const killHandler = withSessionOwnership(ctx, async (req, reply, session) => {
    ...
    try {
      ...
      if (auditLogger) void auditLogger.log(resolveRequestAuditActor(auth, req, 'system'), 'session.kill', `Session killed: ${session.id} (permission=${req.matchedPermission ?? 'kill'})`, session.id, req.tenantId, session.claudeSessionId);
